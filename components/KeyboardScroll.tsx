...
const FRAME_SUFFIX = "_delay-0.04s.webp";

const beats = [
  {
    title: "WpDev Keyboard.",
    subtitle: "Engineered clarity.",
    start: 0,
    end: 0.2,
    align: "center",
  },
  {
    ...
    subtitle: "Every detail, measured.",
    start: 0.2,
    end: 0.5,
    align: "left",
  },
  ...
  },
  {
    ...
    subtitle: "Scroll back to replay.",
    start: 0.82,
    end: 1,
    align: "center",
  },
      ...
      <div className="sticky top-0 flex h-screen w-full items-center justify-center overflow-hidden bg-fog">
        ...
        {!ready && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-fog/90">
            ...
            <p className="mt-4 text-sm tracking-tight text-black/60">
              Loading WpDev sequence… {progressPercent}%
            </p>
          </div>
        )}
...
function StoryBeat({ title, subtitle, start, end, align, scrollYProgress }: StoryBeatProps) {
  ...
  return (
    <motion.div
      ...
      className={`absolute inset-x-0 mx-auto flex max-w-6xl px-6 md:px-10 ${alignClass(align)} ${
        align === "center" ? "top-[12%]" : "top-[18%]"
      }`}
    >
